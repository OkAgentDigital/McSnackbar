...
/// 🧠 MCP Manager — manages Re3Engine (Xcode MCP provider), HivemindRust, and Xcode MCP bridge.
///
/// Responsibilities:
/// - Start/stop/restart Re3Engine (replaces HivemindRust for Xcode MCP)
...
/// - Binds services to localhost only for security
...
///
/// Socket Map:
///   ~/.local/share/udos/mcp/re3engine.sock — Re3Engine MCP (Xcode tools, vault, chat)
///
class MCPManager: ObservableObject {
    ...
    @Published private(set) var re3engineStatus: MCPProcessStatus = .stopped
    ...
    @Published private(set) var re3enginePID: Int32 = 0
    @Published private(set) var lastHealthCheck: Date?
    @Published private(set) var hivemindPort: UInt16 = 3010
    @Published private(set) var re3engineHttpPort: UInt16 = 3011
    ...
    private let re3engineSocketPath = "~/.local/share/udos/mcp/re3engine.sock"

    private var hivemindProcess: Process?
    private var re3engineProcess: Process?
    ...
    // MARK: - Re3Engine Process Management

    /// Start Re3Engine (replaces HivemindRust for Xcode MCP tools).
    /// Re3Engine provides XcodeBuild, XcodeTest, XcodeClean, XcodeOpen, etc.
    func startRe3Engine() {
        guard re3engineStatus != .running else {
            print("⚠️ Re3Engine already running")
            return
        }

        // Try to find the Re3Engine binary
        guard let binaryPath = findRe3EngineBinary() else {
            DispatchQueue.main.async {
                self.re3engineStatus = .error("Binary not found")
            }
            print("❌ Re3Engine binary not found. Build it first: cd ~/Code/OkAgentDigital/Re3Engine && cargo build --release")
            return
        }

        let process = Process()
        process.executableURL = URL(fileURLWithPath: binaryPath)
        // Start with HTTP MCP server enabled on port 3011 (alongside HivemindRust on 3010)
        process.arguments = ["--status", "--http", "--port", "3011"]
        process.currentDirectoryURL = URL(fileURLWithPath: NSString(string: "~/Code/OkAgentDigital/Re3Engine").expandingTildeInPath)
        ...
        outputPipe.fileHandleForReading.readabilityHandler = { handle in
            let data = handle.availableData
            if let line = String(data: data, encoding: .utf8), !line.isEmpty {
                print("🔮 Re3Engine: \(line.trimmingCharacters(in: .whitespacesAndNewlines))")
            }
        }
        errorPipe.fileHandleForReading.readabilityHandler = { handle in
            let data = handle.availableData
        ...
        process.terminationHandler = { [weak self] proc in
            DispatchQueue.main.async {
                self?.re3engineStatus = .stopped
                self?.re3enginePID = 0
                print("🔮 Re3Engine process exited (code: \(proc.terminationStatus))")
            }
        }

        do {
            try process.run()
            re3engineProcess = process
            re3enginePID = process.processIdentifier

            DispatchQueue.main.async {
                self.re3engineStatus = .running
            }

            print("✅ Re3Engine started (PID: \(process.processIdentifier))")
            print("🔌 Xcode MCP socket: \(NSString(string: re3engineSocketPath).expandingTildeInPath)")
            print("🌐 HTTP MCP: http://localhost:\(re3engineHttpPort)/mcp")

            // Retry loop: check socket + HTTP endpoint every 500ms for up to 10s
            DispatchQueue.global().async { [weak self] in
                guard let self = self else { return }
                let socketPath = NSString(string: self.re3engineSocketPath).expandingTildeInPath
                let healthURL = "http://localhost:\(self.re3engineHttpPort)/health"

                for attempt in 1...20 {
                    Thread.sleep(forTimeInterval: 0.5)

                    let socketReady = self.fileManager.fileExists(atPath: socketPath)
                    var httpReady = false
                    ...
                    if socketReady && httpReady {
                        print("✅ Re3Engine ready (socket + HTTP confirmed after \(Double(attempt) * 0.5)s)")
                        DispatchQueue.main.async {
                            self.re3engineStatus = .running
                        }
                        return
                    ...
                    if attempt % 4 == 0 {
                        print("⏳ Waiting for Re3Engine... (attempt \(attempt)/20)")
                    }
                }

                // Timeout — check what's available
                let socketReady = self.fileManager.fileExists(atPath: socketPath)
                print("⚠️ Re3Engine startup timeout — socket: \(socketReady), HTTP: check logs")
                DispatchQueue.main.async {
                    self.re3engineStatus = socketReady ? .running : .error("Startup timeout")
                }
            }
        } catch {
            DispatchQueue.main.async {
                self.re3engineStatus = .error(error.localizedDescription)
            }
            print("❌ Failed to start Re3Engine: \(error.localizedDescription)")
        }
    }

    /// Stop Re3Engine process.
    func stopRe3Engine() {
        guard let process = re3engineProcess, process.isRunning else {
            // Try by PID
            if re3enginePID > 0 {
                kill(re3enginePID, SIGTERM)
                DispatchQueue.main.async {
                    self.re3engineStatus = .stopped
                    self.re3enginePID = 0
                }
                print("🛑 Re3Engine (PID: \(re3enginePID)) sent SIGTERM")
                return
            }
            print("⚠️ No Re3Engine process to stop")
            return
        }

        process.terminate()
        re3engineProcess = nil
        re3enginePID = 0

        DispatchQueue.main.async {
            self.re3engineStatus = .stopped
        }
        print("🛑 Re3Engine stopped")
    }

    /// Restart Re3Engine.
    func restartRe3Engine() {
        print("🔄 Restarting Re3Engine...")
        stopRe3Engine()
    ...
    /// Check if Re3Engine is available (binary exists).
    func isRe3EngineAvailable() -> Bool {
        return findRe3EngineBinary() != nil
    }

    /// Find the Re3Engine binary in likely locations.
    private func findRe3EngineBinary() -> String? {
        let candidates = [
            "~/Code/OkAgentDigital/Re3Engine/target/release/re3engine",
            "~/Code/OkAgentDigital/Re3Engine/target/debug/re3engine",
            "~/.cargo/bin/re3engine",
            "/usr/local/bin/re3engine",
            "/opt/homebrew/bin/re3engine"
        ]
    ...
    /// Send an MCP request to Re3Engine via its Unix socket.
    /// - Parameters:
    ///   - method: The MCP method name (e.g., "XcodeBuild", "XcodeTest")
    ///   - params: The parameters as a JSON dictionary
    /// - Returns: The response string, or nil on failure
    func sendRe3EngineRequest(method: String, params: [String: Any] = [:]) -> String? {
        let socketPath = NSString(string: re3engineSocketPath).expandingTildeInPath
        guard fileManager.fileExists(atPath: socketPath) else {
            print("❌ Re3Engine socket not found at \(socketPath)")
            return nil
        }

        // Build the MCP request: {"MethodName": params}
        let requestDict: [String: Any] = [method: params]
        guard let requestData = try? JSONSerialization.data(withJSONObject: requestDict),
            ...
            print("❌ Failed to serialize MCP request")
            return nil
        }

        // Use a shell command to send via the socket (nc/socat)
        let task = Process()
        task.executableURL = URL(fileURLWithPath: "/bin/bash")
        task.arguments = ["-c", "echo '\(requestString)' | nc -U '\(socketPath)'"]
        ...
        do {
            ...
            print("❌ Failed to send MCP request to Re3Engine: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    /// Re3Engine (Xcode tools), and HivemindRust.
    ///
    /// Protocol notes:
    ///   - Snackbar uses HTTP JSON-RPC (type: "http") — direct POST requests
    ///   - Re3Engine uses Unix socket MCP protocol (type: "socket") — Xcode build/test/run tools
    ///     Also serves HTTP JSON-RPC at /mcp when started with --http flag
        ...
        let config: [String: Any] = [
            "mcpServers": [
                ...
                "re3engine": [
                    "type": "socket",
                    "path": socketPath,
                    "description": "Xcode build, test, clean, open, and project management tools via Re3Engine"
                ],
                "re3engine-http": [
                    "type": "http",
                    "url": "http://localhost:\(re3engineHttpPort)",
                    "description": "Re3Engine HTTP MCP server (Xcode tools via JSON-RPC)"
                ],
                "hivemind": [
                    "type": "sse",
    ...
    func getStatusSummary() -> String {
        ...
        lines.append("─────────────────────")
        lines.append("Re3Engine (Xcode MCP):  \(re3engineStatus.displayText)")
