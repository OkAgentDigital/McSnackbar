...
class SnackbarAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Start Re3Engine (Xcode MCP tools via HTTP on port 3011)
        if mcpManager.isRe3EngineAvailable() {
            mcpManager.startRe3Engine()
        } else {
            print("⚠️ Re3Engine binary not found. Xcode MCP tools won't be available.")
            print("   Build it: cd ~/Code/OkAgentDigital/Re3Engine && cargo build --release")
        }
        
        // Perform an async health check on the Re3Engine MCP client
        Task {
            let mcpClient = MCPClient()
            let healthy = await mcpClient.isHealthy()
            if healthy {
                print("✅ MCPClient connected to Re3Engine at \(mcpClient.baseURL)")
                if let tools = try? await mcpClient.listTools() {
                    print("📋 MCPClient discovered \(tools.count) tools")
                }
            } else {
                print("⚠️ MCPClient: Re3Engine not reachable at \(mcpClient.baseURL)")
            }
        }
