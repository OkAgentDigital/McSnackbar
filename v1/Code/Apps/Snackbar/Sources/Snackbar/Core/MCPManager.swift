...
class MCPManager: ObservableObject {
    ...
    private let xcodeMCPStdioCommand = "xcrun mcpbridge"
    ...
    func getXcodeBridgeInfo() -> [String: Any] {
        [
            ...
            "command": xcodeMCPStdioCommand,
            "status": xcodeBridgeStatus.displayText,
            "mcp_servers": listConfiguredMCPServers()
        ]
    }
    ...
    func checkUbuntuMCP(host: String = "192.168.20.11", port: Int = 3010) {
        ...
        do {
            ...
            DispatchQueue.main.async {
                if task.terminationStatus == 0 && output.contains("200") {
                    self.ubuntuMCPStatus = .running
                } else if task.terminationStatus == 0 {
                    self.ubuntuMCPStatus = .error("HTTP \(output.trimmingCharacters(in: .whitespacesAndNewlines))")
                } else {
    ...
    /// and HivemindRust. This can be written to Xcode's config or shown to the user.
